...
if (messageDetail) {
	...
	const runMessageDetail = function (
	   ...
	   Returns '' when the message has no files. */
		...
		/* Build a tree from flat messages using r_messageReplies_c_c2m0MessageId */
		function buildMessageTree(messages, opId) {
			...
			messages.forEach((msg) => {
				let parentId = msg.r_messageReplies_c_c2m0MessageId || 0;
			...
			Liferay.Util.fetch(
				...
				.then((data) => {
					...
					items.forEach(
						({
							id: voteId,
							r_messageVotes_c_c2m0MessageId: msgId,
							value: voteValue,
						}) => {
		...
		function handleMarkAnswer(messageId, isCurrentlyAnswer) {
			if (isCurrentlyAnswer) {
				...
				/* Unmark this answer */
				Liferay.Util.fetch(
					portalURL + '/o/c/c2m0messages/' + messageId,
					{
				...
				if (currentAnswerId && currentAnswerId !== messageId) {
					chain = Liferay.Util.fetch(
						portalURL + '/o/c/c2m0messages/' + currentAnswerId,
						{
							body: JSON.stringify({answer: false}),
							headers,
							method: 'PATCH',
						...
						Liferay.Util.fetch(
							portalURL +
								...
								encodeURIComponent(
									'creatorId eq ' +
										currentUserId +
										" and r_threadSuspiciousActivities_c_c2m0ThreadERC eq '" +
										externalReferenceCode +
										"'"
								) +
								'&pageSize=1',
