...
if (messageComposer) {
	...
	const loadExistingAttachments = function (messageId) {
		...
		const filter = encodeURIComponent(
			"r_messageAttachments_c_c2m0MessageId eq '" + messageId + "'"
		);
		Liferay.Util.fetch(
			portalURL +
				'/o/c/c2m0attachments/scopes/' +
				scopeGroupId +
		...
		return Promise.all(
			removedAttachmentIds.map((id) => {
				return Liferay.Util.fetch(
					portalURL + '/o/c/c2m0attachments/' + id,
					{
						headers,
						method: 'DELETE',
					}
				...
				return fileToBase64(file)
					.then((base64) => {
						const body = {
							...
							r_messageAttachments_c_c2m0MessageId: parseInt(
								messageId,
								10
							),
						};
		...
		form.addEventListener('submit', (event) => {
			...
			if (isEditMode) {
				...
				if (editIsOp) {
					...
					promises.push(
						Liferay.Util.fetch(
							portalURL + '/o/c/c2m0messages/' + editMessageId,
							{
								body: JSON.stringify({
									body,
									subject,
				...
				const replyPayload = {
					body,
					format: 'html',
					r_messageReplies_c_c2m0MessageId: parentMessageId
						? parseInt(parentMessageId, 10)
						: 0,
					r_threadMessages_c_c2m0ThreadId: parseInt(messageId, 10),
				...
				Liferay.Util.fetch(
					portalURL + '/o/c/c2m0threads/scopes/' + scopeGroupId,
					{
